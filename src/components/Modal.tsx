...
export default function Modal(props: { open: boolean; title: string; children: React.ReactNode; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center p-4 safe-bottom">
      ...
      <div className="relative w-full max-w-[440px] overflow-hidden rounded-[28px] border border-white/10 bg-[#0E0E14]/92 shadow-glow">
        <div className="flex items-center justify-between px-5 pb-3 pt-5">
          <div className="text-[15px] font-semibold text-white">{props.title}</div>
          <button onClick={props.onClose} className="rounded-full border border-white/10 bg-white/5 px-3 py-1.5 text-[12px] text-white/80">
            Fermer
          </button>
        </div>
